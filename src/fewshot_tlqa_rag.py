...
def parse_args():
    ...
    parser.add_argument('--train-emb', default='../data/train_TLQA_emb.json', help='Path to the train embeddings')
...
def main():
    ...
    with open(args.train_emb, 'r') as f:
        train_emb_data = json.load(f)
    train_questions = get_transfer_questions(train_set)
    train_questions_emb = np.array(train_emb_data['embeddings'])
